...
class Assistant:
    ...
    def __init__(self):
        ...
        self.client = None
        if api_key := getattr(Config, 'ANTHROPIC_API_KEY', None):
            self.client = anthropic.Anthropic(api_key=api_key)
        else:
            self.console.print("[yellow]Warning: ANTHROPIC_API_KEY not found. Running in tools-only mode.[/yellow]")
...
[cyan]📤 Result:[/cyan] {cleaned_result}"""
    ...
    def reset(self):
        ...
        This version is safe for web server environments and does not print to console.
        """
