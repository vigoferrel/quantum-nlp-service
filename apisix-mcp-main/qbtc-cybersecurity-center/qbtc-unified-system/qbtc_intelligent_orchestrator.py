...
class QbtcIntelligentOrchestrator:
    ...
    async def activate_apisix_mode(self, health_report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("APISIX detectado como saludable, activando modo APISIX...")
        
        results = {
            'mode': 'apisix_native',
            'apisix_admin_url': self.apisix_docker_admin,
            'apisix_gateway_url': self.apisix_docker_gateway,
            'fallback_proxy_active': False
        }
        
        # Verificar configuración APISIX
        try:
            response = requests.get(f"{self.apisix_docker_admin}/routes", 
                                  headers={"X-API-KEY": "qbtc-888hz-key"}, timeout=5)
            if response.status_code == 200:
                existing_routes = response.json()
                results['existing_routes'] = len(existing_routes.get('list', []))
                logger.info(f"APISIX activo con {results['existing_routes']} rutas existentes")
            else:
                results['apisix_config_error'] = f"Status code: {response.status_code}"
        except Exception as e:
            results['apisix_connection_error'] = str(e)
    ...
    async def activate_hybrid_mode(self, health_report: Dict[str, Any]) -> Dict[str, Any]:
        """Activar modo híbrido para máxima compatibilidad"""
        
        logger.info("Activando modo híbrido...")
        
        # Intentar APISIX primero
        apisix_results = await self.activate_apisix_mode(health_report)
        
        # Iniciar fallback como respaldo
        fallback_results = await self.activate_fallback_mode(health_report)
        
        return {
            'mode': 'hybrid',
            'primary_gateway': 'apisix' if apisix_results.get('existing_routes', 0) > 0 else 'fallback',
            'apisix_status': apisix_results,
            'fallback_status': fallback_results
        }
    ...
    async def enhanced_service_discovery(self) -> List[Dict[str, Any]]:
        ...
        # 1. Port scanning de servicios conocidos
        for port, service_info in self.critical_services.items():
        ...
        # 2. Detección de procesos cuánticos
        quantum_processes = await self.detect_quantum_processes()
        discovered_services.extend(quantum_processes)
        
        # 3. Análisis de colas RabbitMQ (si está disponible)
        if await self.check_port_active(15672):
            rabbitmq_services = await self.analyze_rabbitmq_services()
            discovered_services.extend(rabbitmq_services)
        
        # Deduplicar servicios
        unique_services = {}
        for service in discovered_services:
            key = f"{service['name']}:{service.get('port', 'unknown')}"
            if key not in unique_services:
                unique_services[key] = service
        
        return list(unique_services.values())
    
    async def detect_quantum_processes(self) -> List[Dict[str, Any]]:
        """Detectar procesos con nombres cuánticos"""
        
        quantum_keywords = ["quantum", "qbtc", "vigoleon", "llm", "api", "fastapi", "uvicorn"]
        processes = []
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'connections']):
                try:
                    ...
                    if any(keyword in cmdline.lower() for keyword in quantum_keywords):
                        ...
                        process_service = {
                            'name': f"process_{proc.info['name']}",
                            'type': 'process_detected',
                            'pid': proc.info['pid'],
                            'cmdline': cmdline[:100],  # Truncar para logs
                            'port': port,
                            'url': f"http://127.0.0.1:{port}" if port else None
                        }
                        processes.append(process_service)
                        
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
        except Exception as e:
            logger.warning(f"Error en detección de procesos: {e}")
        
        return processes
    
    async def analyze_rabbitmq_services(self) -> List[Dict[str, Any]]:
        """Analizar servicios a través de RabbitMQ"""
        
        services = []
        try:
            # Intentar conectar a RabbitMQ Management API
            response = requests.get("http://127.0.0.1:15672/api/queues", 
                                  auth=('quantum_user', 'VIGOLEONROCKS_888HZ'), timeout=5)
            ...
            if response.status_code == 200:
                ...
                for queue in queues:
                    ...
                    if any(keyword in queue_name.lower() for keyword in ['quantum', 'qbtc', 'experiment']):
                        service = {
                            'name': f"rabbitmq_queue_{queue_name}",
                            'type': 'rabbitmq_queue',
            ...
            logger.warning(f"Error analizando RabbitMQ: {e}")
    ...
    async def configure_routes_intelligent(self, services: List[Dict[str, Any]], strategy: str) -> Dict[str, Any]:
        ...
        if strategy == "apisix_available" and await self.check_port_active(9180):
            # Usar APISIX nativo
            results = await self.configure_apisix_routes(services)
            results['method'] = 'apisix_native'
            
        elif self.fallback_proxy_active and await self.check_port_active(9079):
            # Usar proxy fallback
    ...
    async def configure_apisix_routes(self, services: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Configurar rutas APISIX (método original mejorado)"""
        
        results = {'routes': [], 'upstreams': [], 'errors': []}
        admin_headers = {"X-API-KEY": "qbtc-888hz-key", "Content-Type": "application/json"}
        
        for service in services:
            if not service.get('url') or not service.get('port'):
                continue
            ...
            try:
                ...
                # Configurar upstream
                upstream_id = f"upstream-{service_name}"
                upstream_config = {
                    "type": "roundrobin",
                    "nodes": {service_url.replace('http://', ''): 1},
                    "timeout": {"connect": 5, "send": 5, "read": 30}
                }
                
                upstream_response = requests.put(
                    f"{self.apisix_docker_admin}/upstreams/{upstream_id}",
                    headers=admin_headers, json=upstream_config, timeout=10
                )
                ...
                # Configurar ruta
                route_id = f"route-{service_name}"
                route_config = {
                    "name": f"QBTC Intelligent Route for {service_name}",
                    "uri": f"/api/{service_name.replace('_', '-')}/*",
                    ...
                    "labels": {
                        ...
                        "vigoleonrocks_intelligent": "true",
                ...
                route_response = requests.put(
                    ...
                    headers=admin_headers, json=route_config, timeout=10
                )
                
                if route_response.status_code in [200, 201]:
                    results['routes'].append(route_id)
                    logger.info(f"Ruta APISIX configurada: {route_id} -> {service_url}")
                else:
                    error_msg = f"Error configurando ruta APISIX {route_id}: {route_response.status_code}"
                    results['errors'].append(error_msg)
                    
            except Exception as e:
                error_msg = f"Error configurando servicio {service.get('name', 'unknown')}: {e}"
                results['errors'].append(error_msg)
    ...
    async def intelligent_connectivity_tests(self, strategy: str) -> Dict[str, Dict[str, Any]]:
        ...
        # Test del gateway principal
        if strategy == "apisix_available":
            tests['primary_gateway'] = await self.test_endpoint(self.apisix_docker_gateway, "APISIX Gateway")
            tests['admin_api'] = await self.test_endpoint(
                f"{self.apisix_docker_admin}/routes", "APISIX Admin API",
                headers={"X-API-KEY": "qbtc-888hz-key"}
            )
        elif self.fallback_proxy_active:
            tests['primary_gateway'] = await self.test_endpoint(f"{self.fallback_proxy_url}/health", "Fallback Proxy")
            tests['proxy_metrics'] = await self.test_endpoint(f"{self.fallback_proxy_url}/metrics", "Proxy Metrics")
    ...
    async def calculate_enhanced_performance_score(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if strategy == "apisix_available":
            gateway_score = 25
        elif strategy == "fallback_required" and self.fallback_proxy_active:
            gateway_score = 20  # Fallback funcional vale un poco menos
        else:
...
return results
    ...
    async def calculate_enhanced_performance_score(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'improvement_suggestions': await self.generate_improvement_suggestions(score_components, strategy)
        }
    
    async def generate_improvement_suggestions(self, score_components: Dict[str, float], strategy: str) -> List[str]:
        """Generar sugerencias de mejora específicas"""
        suggestions = []
        
        if score_components.get('infrastructure', 0) < 25:
            suggestions.append("Activar más servicios de infraestructura (RabbitMQ, Redis, Supabase)")
        
        if score_components.get('gateway', 0) < 20:
            suggestions.append("Verificar y reparar gateway (APISIX o proxy fallback)")
        
        if score_components.get('service_discovery', 0) < 15:
            suggestions.append("Iniciar más servicios API para mejorar discovery")
        
        if score_components.get('routes', 0) < 12:
            suggestions.append("Configurar más rutas para servicios activos")
        
        if score_components.get('connectivity', 0) < 8:
            suggestions.append("Verificar conectividad de red y health checks")
        
        if strategy == "fallback_required":
            suggestions.append("Considerar reactivar APISIX Docker para mejor rendimiento")
    ...
    async def start_auto_healing_system(self) -> Dict[str, Any]:
        ...
        # Iniciar tarea de auto-sanación en background
        asyncio.create_task(self.auto_healing_loop())
        
        logger.info("Sistema de auto-sanación iniciado")
        return healing_status
    
    async def auto_healing_loop(self):
        """Loop de auto-sanación continua"""
        
        while True:
            try:
                await asyncio.sleep(60)  # Verificar cada minuto
                
                logger.debug("Ejecutando verificación de auto-sanación...")
                
                # Verificar servicios críticos
                for port, service_info in self.critical_services.items():
                    if service_info['essential'] and not await self.check_port_active(port):
                        logger.warning(f"Servicio crítico caído detectado: {service_info['name']}")
                        await self.heal_service(service_info)
                
                # Verificar proxy fallback si está activo
                if self.fallback_proxy_active and not await self.check_port_active(9079):
                    logger.warning("Proxy fallback caído, reintentando...")
                    await self.restart_fallback_proxy()
                
                # Verificar estado general del sistema cada 5 minutos
                if int(time.time()) % 300 == 0:  # Cada 5 minutos
                    await self.comprehensive_system_check()
                
            except Exception as e:
                logger.error(f"Error en loop de auto-sanación: {e}")
                await asyncio.sleep(30)  # Esperar más tiempo si hay error
    
    async def heal_service(self, service_info: Dict[str, Any]):
        """Intentar sanar un servicio específico"""
        
        self.performance_metrics['auto_healing_events'] += 1
        service_name = service_info['name']
        
        logger.info(f"Iniciando auto-sanación para: {service_name}")
        
        # Si el servicio tiene comando de inicio, intentar ejecutarlo
        if service_info.get('cmd'):
            try:
                logger.info(f"Ejecutando comando de recuperación: {service_info['cmd']}")
                subprocess.Popen(service_info['cmd'], shell=True)
                
                # Esperar un momento y verificar
                await asyncio.sleep(5)
                
                if await self.check_port_active(service_info.get('port')):
                    logger.info(f"Auto-sanación exitosa para: {service_name}")
                else:
                    logger.warning(f"Auto-sanación falló para: {service_name}")
                    
            except Exception as e:
                logger.error(f"Error en auto-sanación de {service_name}: {e}")
    
    async def restart_fallback_proxy(self):
        """Reiniciar proxy fallback"""
        
        try:
            # Terminar proceso anterior si existe
            if self.fallback_proxy_process and self.fallback_proxy_process.poll() is None:
                self.fallback_proxy_process.terminate()
            ...
            if await self.check_port_active(9079):
                self.fallback_proxy_active = True
                logger.info("Proxy fallback reiniciado exitosamente")
            else:
                logger.error("Fallo al reiniciar proxy fallback")
                
        except Exception as e:
            logger.error(f"Error reiniciando proxy fallback: {e}")
    
    async def comprehensive_system_check(self):
        """Verificación completa del sistema cada 5 minutos"""
        
        logger.info("Ejecutando verificación completa del sistema...")
        ...
        if cpu_percent > 80:
            logger.warning(f"Alto uso de CPU detectado: {cpu_percent}%")
        
        if memory_percent > 80:
            logger.warning(f"Alto uso de memoria detectado: {memory_percent}%")
    ...
    async def check_recent_errors(self):
        ...
        try:
            ...
            if logs_dir.exists():
                ...
                recent_logs = sorted(logs_dir.glob("monitoring_*.json"), key=lambda x: x.stat().st_mtime)[-3:]
                ...
                if error_count > 10:
                    logger.warning(f"Muchos errores detectados en logs recientes: {error_count}")
                    
        except Exception as e:
            logger.debug(f"Error verificando logs: {e}")
