#!/usr/bin/env python3
"""
🧠 CIO MULTIMODAL EXTENSION - VERSIÓN FINAL CORREGIDA
Extensión multimodal para el sistema CIO existente
Corregido el problema de inicialización asíncrona y fallback
"""
...
class CIOMultimodalExtensionFinal:
    """
    Extensión multimodal para el sistema CIO existente - VERSIÓN FINAL CORREGIDA
    """
    
    def __init__(self):
        # Inicializar el cerebro CIO existente de forma segura
        self.cio_brain = None
    ...
    def _initialize_cio_brain_safely(self):
        ...
        """
        try:
            # Crear el cerebro CIO sin verificación asíncrona
            self.cio_brain = QBTCQuantumBrainLeonardo(brain_id="multimodal_cio_final")
            ...
            self.cio_brain = None
            logger.info("🔄 Sistema funcionando en modo fallback con OpenRouter")
    ...
    async def _fallback_multimodal_process(self, query: str, image_context: str) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("🔄 Usando modo fallback con OpenRouter")
            ...
            logger.info(f"📡 Enviando consulta a OpenRouter: {query[:50]}...")
            ...
            logger.info(f"📡 Respuesta OpenRouter: {response.status_code}")
            
            if response.status_code == 200:
                result = response.json()
                response_text = result['choices'][0]['message']['content']
                
                logger.info(f"✅ Respuesta recibida: {response_text[:50]}...")
                ...
                logger.error(f"❌ Error OpenRouter HTTP: {response.status_code}")
                logger.error(f"❌ Respuesta: {response.text}")
                return {
                    'error': f"Error OpenRouter: {response.status_code}",
                    'query': query,
                    'response': f'Error en procesamiento: HTTP {response.status_code}'
                }
            ...
            return {
                'error': str(e),
                'query': query,
                'response': f'Error en procesamiento de fallback: {str(e)}'
            }
...
# Función de prueba
async def test_cio_final():
    """Probar la extensión CIO final corregida"""
    print("🧠 PROBANDO CIO MULTIMODAL FINAL CORREGIDO")
    print("=" * 60)
    
    extension = CIOMultimodalExtensionFinal()
