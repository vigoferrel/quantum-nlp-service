...
class VIGOLEONROCKSArchitectReorganizer:
    ...
    def execute_reorganization(self, plan: Dict, dry_run: bool = True) -> Dict:
        ...
        if not dry_run:
            ...
            try:
                ...
                error_msg = f"Error al crear estructura objetivo: {str(e)}"
                self.logger.error(error_msg)
                results['errors'].append(error_msg)
                results['status'] = 'failed'
                return results
    ...
    def _create_backup(self):
        ...
        try:
            # Usar una ruta con slash normal en lugar de backslash para evitar problemas en Windows
            backup_dir_str = str(self.project_root / "_backup_original_structure").replace('\\', '/')
            self.backup_dir = Path(backup_dir_str)
            
            if self.backup_dir.exists():
                shutil.rmtree(self.backup_dir)
            
            # Primero crear el directorio de backup
            ...
            for item in self.project_root.iterdir():
                ...
                try:
                    if item.is_dir():
                        dest_dir = self.backup_dir / item.name
                        dest_dir.mkdir(parents=True, exist_ok=True)
                        
                        # Copiar contenido de cada subdirectorio manualmente
                        for sub_item in item.rglob('*'):
                            ...
                            if sub_item.is_dir():
                                ...
                                try:
                                    ...
                                    self.logger.warning(f"No se pudo copiar {sub_item}: {e}")
                    else:
                        try:
                            shutil.copy2(item, self.backup_dir)
                        except (PermissionError, OSError) as e:
                            self.logger.warning(f"No se pudo copiar {item}: {e}")
                
                except Exception as e:
                    self.logger.warning(f"Error al procesar {item}: {e}")
            
            self.logger.info("✅ Backup completado con éxito")
        
        except Exception as e:
            self.logger.error(f"❌ Error crítico en backup: {e}")
            raise
    ...
    def _create_target_structure(self):
        ...
        try:
            ...
            self.new_structure_dir.mkdir(parents=True, exist_ok=True)
...
"""
            ...
            self.logger.info("✅ Estructura objetivo creada con éxito")
        
        except Exception as e:
            self.logger.error(f"❌ Error al crear estructura objetivo: {e}")
            raise
    ...
    def _generate_ascii_report(self, analysis: Dict, plan: Dict, results: Dict):
        ...
        report_lines.extend([
            "",
            "=" * 80,
            "Fin del reporte - VIGOLEONROCKS Architect Reorganizer v2.0.1",
            "=" * 80
        ])
...
def reorganize_vigoleonrocks(dry_run=True):
    ...
    if not dry_run:
        ...
        # Crear el directorio de backup primero
        backup_dir.mkdir(parents=True, exist_ok=True)
        
        # Copiar archivos y directorios, excluyendo algunos patrones
        for item in project_root.iterdir():
            if item.name.startswith('_backup_') or item.name.startswith('_new_optimized_') or item.name == '.git':
                continue
                
            try:
                if item.is_dir():
                    print(f'  Copiando directorio: {{item.name}}')
                    shutil.copytree(item, backup_dir / item.name)
                else:
                    print(f'  Copiando archivo: {{item.name}}')
    ...
    # Estructura de movimientos de archivos
    movements = json.loads('''{component_mappings}''')
    
    for src_path, dst_path in movements.items():
        src = project_root / src_path
        dst = new_structure_dir / dst_path / src_path.split('/')[-1]
    ...
    print(f'📁 Nueva estructura disponible en: {{new_structure_dir}}')

if __name__ == '__main__':
    import sys
    dry_run = '--execute' not in sys.argv
    if not dry_run:
        confirm = input("⚠️ ¿Estás seguro de ejecutar la reorganización real? (si/no): ")
...
"""
        ...
        component_mappings_str = json.dumps(plan['component_mappings'], indent=4)
        python_script = python_script.replace("{component_mappings}", component_mappings_str)
